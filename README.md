# 牛肉项目雷达 / Backend Agent Resume Scout

> 程序员牛肉出品：一个帮后端和 AI Agent 求职者，从 GitHub 里筛出真正能写进简历、能经得起面试追问的项目选择器。

它不是简单的 GitHub 项目合集，也不是按 star 数排序的项目推荐器。

它会根据用户的求职方向，从 GitHub / Web 中搜索传统软件后端项目和业务型 AI Agent 项目，筛掉 demo、薄封装、浅层 CRUD、浏览器插件和纯框架项目，优先选择有业务闭环、状态流转、持久化、异步链路、权限边界和面试可讲性的项目。

最终推荐前，它会拉取真实源码到本地，基于源码证据提取功能点和技术难点，再生成可用于简历准备的 Markdown / PDF 项目包。
...
```text
仓库名：backend-agent-resume-scout
产品名：牛肉项目雷达
副标题：Source-verified backend & AI Agent project scout for resume building.
```

其中 `backend-agent-resume-scout` 更适合作为 GitHub 仓库名，`牛肉项目雷达` 更适合作为中文传播名。
